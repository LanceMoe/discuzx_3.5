...
if (!$row1 = DB::fetch_first("SHOW COLUMNS FROM " . DB::table('dsu_paulsign') . " LIKE 'lasted'")) {
  DB::query("ALTER TABLE " . DB::table('dsu_paulsign') . " ADD lasted int(5) NOT NULL DEFAULT '0'");
}
if (!$row2 = DB::fetch_first("SHOW COLUMNS FROM " . DB::table('common_stat') . " LIKE 'paulsign'")) {
  DB::query("ALTER TABLE " . DB::table('common_stat') . " ADD paulsign int(10) UNSIGNED NOT NULL DEFAULT '0'");
}
