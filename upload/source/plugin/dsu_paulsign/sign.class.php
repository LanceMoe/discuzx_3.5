...
class plugin_dsu_paulsign
{
  function dsu_signtz()
  {
    ...
    $fastreplytexts = array_values(array_filter(explode('/hhf/', str_replace(array("\r\n", "\n", "\r"), '/hhf/', $_G['cache']['plugin']['dsu_paulsign']['fastreplytext'])), 'strlen'));
    $emots = dsu_paulsign_emots($_G['setting']['paulsign_emot']);
    if (in_array($_G['groupid'], dsu_paulsign_unserialize_array($_G['cache']['plugin']['dsu_paulsign']['autosign_ug']), true) && $fastreplytexts && $emots) {
      return '<script type="text/javascript">showWindow(\'dsu_paulsign\', \'plugin.php?id=dsu_paulsign:sign&operation=qiandao&formhash=' . FORMHASH . '&qdmode=2&fastreply=' . array_rand($fastreplytexts) . '&qdxq=' . array_rand($emots) . '\');</script>';
    }
  ...
  function global_usernav_extra2()
  {
    ...
    if ($var['ajax_sign'] && $var['ifopen'] && !$show_message && !defined('IN_dsu_paulsign') && !defined('IN_dsu_paulsc') && empty($_GET['infloat']) && !$_G['inajax'] && $_G['uid'] && !in_array((int)$_G['uid'], $banuids, true) && in_array((int)$_G['groupid'], $groupids, true)) {
          ...
          if ($qiandaodb['time'] < $tdtime) {
            if ($var['timeopen']) {
              if (!($htime < $var['stime']) && !($htime > $var['ftime'])) {
                return '<span class="pipe">|</span><a href="javascript:;" onclick="showWindow(\'dsu_paulsign\', \'plugin.php?id=dsu_paulsign:sign&' . FORMHASH . '\')" style="color:red;">' . lang('plugin/dsu_paulsign', 'encore_01') . '</a> ';
              }
            } else {
              return '<span class="pipe">|</span><a href="javascript:;" onclick="showWindow(\'dsu_paulsign\', \'plugin.php?id=dsu_paulsign:sign&' . FORMHASH . '\')" style="color:red;">' . lang('plugin/dsu_paulsign', 'encore_01') . '</a> ';
            }
  ...
  function global_footer()
  {
    ...
    $autosignids = array_values(array_filter(array_map('intval', dsu_paulsign_unserialize_array($var['autosign_ug'])), function($gid) { return $gid > 0; }));
    $groupids = array_values(array_filter(array_map('intval', dsu_paulsign_unserialize_array($var['groups'])), function($gid) { return $gid > 0; }));
    $banuids = array_values(array_filter(array_map('intval', explode(',', (string)$var['ban'])), function($uid) { return $uid > 0; }));
    $tdtime = gmmktime(0, 0, 0, dgmdate($_G['timestamp'], 'n', $var['tos']), dgmdate($_G['timestamp'], 'j', $var['tos']), dgmdate($_G['timestamp'], 'Y', $var['tos'])) - $var['tos'] * 3600;
    $allowmem = memory('check');
    if ($var['ifopen'] && $var['ftopen'] && !$show_message && !defined('IN_dsu_paulsign') && !defined('IN_dsu_paulsc') && empty($_GET['infloat']) && !$_G['inajax'] && $_G['uid'] && (in_array((int)$_G['groupid'], $tzgroupids, true) || in_array((int)$_G['groupid'], $autosignids, true)) && !in_array((int)$_G['uid'], $banuids, true) && in_array((int)$_G['groupid'], $groupids, true)) {
      if ($allowmem && $var['mcacheopen']) {
        $signtime = memory('get', 'dsu_pualsign_' . $_G['uid']);
      }
      $htime = dgmdate($_G['timestamp'], 'H', $var['tos']);
...
class plugin_dsu_paulsign_home extends plugin_dsu_paulsign
{
  function space_home_side_top()
  {
    global $_G;
    if (!$_G['uid']) {
      $style = 'check-in-default-home';
      $qdays = '0';
      $to = 'member.php?mod=logging&action=login';
      $msg = lang('plugin/dsu_paulsign', 'sb_01');
    } else {
      $tdtime = gmmktime(0, 0, 0, dgmdate($_G['timestamp'], 'n', $_G['cache']['plugin']['dsu_paulsign']['tos']), dgmdate($_G['timestamp'], 'j', $_G['cache']['plugin']['dsu_paulsign']['tos']), dgmdate($_G['timestamp'], 'Y', $_G['cache']['plugin']['dsu_paulsign']['tos'])) - $_G['cache']['plugin']['dsu_paulsign']['tos'] * 3600;
      $qiandaodb = C::t('#dsu_paulsign#dsu_paulsign')->fetch($_G['uid']);
      $qdays = $qiandaodb['days'] ? $qiandaodb['days'] : '0';
      if ($qiandaodb['time'] > $tdtime) {
        $style = 'check-in-disabled-home';
        $msg = lang('plugin/dsu_paulsign', 'sb_02');
      } else {
        $style = 'check-in-default-home';
        $msg = lang('plugin/dsu_paulsign', 'sb_03');
        $to = 'plugin.php?id=dsu_paulsign:sign&' . FORMHASH;
      }
    }
    $to = $to ? ' href="javascript:;" onclick="showWindow(\'dsu_paulsign\', \'' . $to . '\')"' : '';
    return '<div class="mod-check-in-home" style="visibility: visible; ">
<a' . $to . '>
	<div class="check-in-inner-home ' . $style . '" title="' . $msg . '" id="j_checkin_btn">
    	<p class="hide-offscreen-home">' . lang('plugin/dsu_paulsign', 'tsn_03') . '</p>
    	<div class="date-week-home">
        	<div class="dates-home">' . dgmdate(TIMESTAMP, 'm.d') . '</div>
            <div class="week-home">' . dgmdate(TIMESTAMP, 'D') . '</div>
        </div>
        <p class="hide-offscreen-home"></p>
        <div class="checked-num-home" title="' . lang('plugin/dsu_paulsign', 'classn_01') . '' . $qdays . '' . lang('plugin/dsu_paulsign', 'classn_02') . '">' . $qdays . '</div><span class="hide-offscreen-home">' . lang('plugin/dsu_paulsign', 'classn_02') . '</span>
    </div>
</a>
</div>';
  }
  function space_profile_baseinfo_bottom()
  {
    ...
    if ($var['spaceopen']) {
      ...
      list($lv1name, $lv2name, $lv3name, $lv4name, $lv5name, $lv6name, $lv7name, $lv8name, $lv9name, $lv10name, $lvmastername) = array_map('dhtmlspecialchars', array_pad(explode("/hhf/", $nlvtext), 11, ''));
      $qiandaodb = C::t('#dsu_paulsign#dsu_paulsign')->fetch($_GET['uid']);
      if ($qiandaodb) {
        ...
        if ($qiandaodb['days'] >= '1500') {
          ...
          $q['level'] = lang('plugin/dsu_paulsign', 'echo_11') . "<font color=green><b>[LV.10]{$lv10name}</b></font>" . lang('plugin/dsu_paulsign', 'echo_12') . "<font color=#FF0000><b>[LV.Master]{$lvmastername}</b></font>" . lang('plugin/dsu_paulsign', 'echo_13') . "<font color=#FF0000><b>{$q['lvqd']}</b></font>" . lang('plugin/dsu_paulsign', 'echo_14');
...
class plugin_dsu_paulsign_forum extends plugin_dsu_paulsign
{
  function index_side_top()
  {
    ...
    $to = $to ? ' href="javascript:;" onclick="showWindow(\'dsu_paulsign\', \'' . $to . '\')"' : '';
    return '<div class="mod-check-in" style="visibility: visible; ">
<a' . $to . '>
	<div class="check-in-inner ' . $style . '" title="' . $msg . '" id="j_checkin_btn">
    	<p class="hide-offscreen">' . lang('plugin/dsu_paulsign', 'tsn_03') . '</p>
    	<div class="date-week">
        	<div class="dates">' . dgmdate(TIMESTAMP, 'm.d') . '</div>
            <div class="week">' . dgmdate(TIMESTAMP, 'D') . '</div>
        </div>
        <p class="hide-offscreen"></p>
        <div class="checked-num" title="' . lang('plugin/dsu_paulsign', 'classn_01') . '' . $qdays . '' . lang('plugin/dsu_paulsign', 'classn_02') . '">' . $qdays . '</div><span class="hide-offscreen">' . lang('plugin/dsu_paulsign', 'classn_02') . '</span>
    </div>
</a>
</div>';
  }
  ...
  function viewthread_sidetop_output()
  {
    ...
    if (empty($_GET['tid']) || !is_array($postlist) || !$open) {
      return array();
    }
    $emots = dsu_paulsign_emots($_G['setting']['paulsign_emot']);
    ...
    $nlvtext = str_replace(array("\r\n", "\n", "\r"), '/hhf/', $_G['cache']['plugin']['dsu_paulsign']['lvtext']);
    list($lv1name, $lv2name, $lv3name, $lv4name, $lv5name, $lv6name, $lv7name, $lv8name, $lv9name, $lv10name, $lvmastername) = array_map('dhtmlspecialchars', array_pad(explode("/hhf/", $nlvtext), 11, ''));
    foreach (C::t('#dsu_paulsign#dsu_paulsign')->fetch_all($authorids) as $mrc) {
      $days[$mrc['uid']]['days'] = $mrc['days'];
      if (!array_key_exists($mrc['qdxq'], $emots)) {
        $emotkeys = array_keys($emots);
      ...
      $days[$mrc['uid']]['qdxqzw'] = isset($emots[$mrc['qdxq']]['name']) ? $emots[$mrc['qdxq']]['name'] : '';
      $days[] = $mrc;
    }
    $echoq = array();
