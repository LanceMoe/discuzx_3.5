...
默认请求携带 `response_format.type=json_schema`，Schema 要求四个字段全部存在、禁止额外字段，并将 `decision` 排在最后。标题、正文、版块和内容类型使用结构化 JSON 作为独立 `user` 消息发送，避免把用户正文拼入系统 Prompt。只有字段顺序正确且明确返回 `decision: pass` 才视为通过；未知值、缺失字段、空响应、非 JSON 和截断响应均按故障策略处理。
...
已有 `1.0.0` 或 `1.1.0` 安装升级到 `1.2.0` 时，请在插件管理页面执行升级并更新缓存。升级脚本会补齐配置默认值，并给旧日志表增加 `tid`、`pid` 和帖子索引，不会删除已有日志；历史日志没有帖子编号，因此不会显示帖子链接。
