...
管理员 Prompt 作为 `system` 消息。插件额外附加不可覆盖的输出约束，要求模型只返回以下 JSON 之一：
...
标题、正文、版块和内容类型使用结构化 JSON 作为独立 `user` 消息发送，避免把用户正文拼入系统 Prompt。只有合法且明确的 `decision: pass` 才视为通过；未知值、空响应、非 JSON 和截断响应均按故障策略处理。
...
```text
id, request_id, uid, fid, content_type, decision, reason, categories,
confidence, http_status, latency_ms, content_hash, error_code, created_at
```

后台日志页面支持按决策、内容类型和时间查看，提供清理过期日志及手动清空功能。日志清理采用按概率触发的轻量回收，避免每次发帖都执行删除。
