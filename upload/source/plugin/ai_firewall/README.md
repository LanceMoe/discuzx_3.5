...
3. 扫描并安装 `AI 防水墙`，安装脚本会创建配置表和审计日志表。
...
已有旧版本安装升级到 `1.3.0` 时，请在插件管理页面执行升级并更新缓存。升级脚本会补齐 Thinking 开关等配置默认值，并给旧日志表增加 `tid`、`pid` 和帖子索引，不会删除已有配置或日志。已保存的模型名称会继续保留；新安装默认使用 `gpt-5.6-luna`。
